...
function documentp_table(datajson, table){
  ...
  vartable.fnAddData([
    ...
    '<span class="label label-warning">'+data.total_global+'%</span>',
    '$' + data.total_usd.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
    '<span class="label label-success">'+data.presupuesto+'</span>',
    data.fecha_inicio,
    data.fecha_fin,
    data.atraso,
    data.motivo,
    '$' + data.renta_mensual.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
    data.itc,
