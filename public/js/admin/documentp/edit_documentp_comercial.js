...
function generate_table_products(){
  ...
  $.each(equipo_activo, function( i, key ) {
   ...
   $.each(materiales, function( i, key ) {
     ...
     $('#tabla_productos tbody').append('<tr id="' + key.id + '"><td>'
       + key.cant_sug + '</td>'
       + '<td><a id="cant_req" href="" data-type="text" data-pk="'+ key.id + '" data-url="" data-title="cantidad" data-value="' + key.cant_req + '" data-name="cant_req" class="set-cant-req"></a></td><td class="descripcion">'
       + key.descripcion + '</td><td>'
       + key.categoria + '</td><td>'
       + key.codigo + '</td><td>'
       + key.proveedor + '</td><td>'
