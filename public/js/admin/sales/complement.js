  ...
  function fillSelected(data,table){//Llena la tabla del modal con las facturas seleccionadas
    ...
    $.each(data, function(index, status){
      vartable.fnAddData([
        ...
        status.name,
        status.date_due,
        '<span data-toggle="tooltip" title="'+status.uuid+'">'+status.uuid.substr( 0, 10 )+'</span>',
        '<span data-toggle="tooltip" title="'+status.customers+'">'+status.customers.substr( 0, 10 )+'</span>',
        '<span data-toggle="tooltip" title="'+status.currencies+'">'+status.currencies.substr( 0, 4 )+'</span>',
        status.total,
        status.saldo,
